...
public class MaterialManager {
	 ...
	 * 
	 * // stock a l'instant Stock stock_clone = (Stock) stock.clone();
	 * 
	 * // on ajoute les emprunts retournes a la date LinkedList<Loan>
	 * finished_loans = (LinkedList<Loan>) loans .getFinishedLoans(date); for
	 * (Loan emprunt : finished_loans) { MaterialType material =
	 * emprunt.getMaterial().getMaterialType(); Integer quantity =
	 * emprunt.getQuantity(); for (int i = 0; i < quantity; i++)
	 * stock_clone.add(material); }
	 * 
	 * // on retire les emprunts encore actifs a la date LinkedList<Loan>
	 * active_loans = (LinkedList<Loan>) loans .getActiveLoans(date); for (Loan
	 * emprunt : active_loans) { MaterialType material =
	 * emprunt.getMaterial().getMaterialType(); Integer quantity =
	 * emprunt.getQuantity(); for (int i = 0; i < quantity; i++)
	 * stock_clone.remove(material); }
	 * 
	 * // on retire les reservations a la date LinkedList<Loan> reserves =
	 * (LinkedList<Loan>) reservations .getActiveReservations(date); for (Loan
	 * emprunt : reserves) { MaterialType material =
	 * emprunt.getMaterial().getMaterialType(); Integer quantity =
	 * emprunt.getQuantity(); for (int i = 0; i < quantity; i++)
	 * stock_clone.remove(material); }
	 * 
	 * return stock_clone; }
	 * 
	 * public boolean book(MaterialType materialT, Borrower borrower, int
	 * quantity, GregorianCalendar startDate, GregorianCalendar endDate) throws
	 * Exception {
	 * 
	 * 
	 * // stock prevu a la date t Stock predicted_stock;
	 * 
	 * //génération de l'id du futur emprunt; idLoan ++;
	 * 
	 * // on verifie que la quantite de materiel est disponible pendant toute la
	 * duree de l'emprunt GregorianCalendar virtual_date = (GregorianCalendar)
	 * startDate.clone();
	 * 
	 * do{
	 * 
	 ...
	 * l'emprunt if (predicted_stock.getStock(materialT) < quantity) return
	 * false;
	 * 
	 ...
	 * 
	 * // on récupère un materiel du stock Material material =
	 * stock.getObject(materialT.getReference());
	 * 
	 * // on crée l'emprunt Loan loan = new Loan(borrower, material, quantity,
	 * startDate, endDate, false, false); reservations.add(loan);
	 * 
	 * return true; }
	 */
	 ...
	 * 
	 * @param startDate, endDate entered by the user
	 * 
	 * @return the borrowed material between the two dates
	 */
	public Map<MaterialType, ArrayList<Material>> getBorrowedMaterial(
		...
		for (Loan loan : loans.getLoans()) {
			if ((loan.getStartDate().before(startDate) && loan.getEndDate()
					.after(startDate))
					|| loan.getStartDate().equals(startDate)
					|| (loan.getStartDate().after(startDate) && loan
							.getEndDate().before(endDate))
					|| loan.getEndDate().equals(endDate)
					|| (loan.getStartDate().before(endDate) && loan
				...
				if (!loansByMaterialType.containsKey(loan.getMaterials().get(0)
					...
					for (Material material : loan.getMaterials()) {
						loansByMaterialType.get(
								loan.getMaterials().get(0).getMaterialType())
								.add(material);
					}
				}
			}
	 ...
	 * @param endDate
	 *            the end date
	 * @return the available stock between the two dates
	 */
	public Map<MaterialType, ArrayList<Material>> getAvailableStock(
		...
		for (MaterialType materialType : stock.getStock().keySet()) {
			if (loansByMaterialType.get(materialType) == null) {
				...
				if (stock.getStock(materialType).size() > loansByMaterialType
						.get(materialType).size()) {
					ArrayList<Material> availableMaterials = new ArrayList<Material>();
					for (Material material : stock.getStock(materialType)) {
						if (!loansByMaterialType.get(materialType).contains(
								material)) {
	 ...
	 * @return true, if successful
	 */
	public Loan book(int ref, User connectedUser, GregorianCalendar startDate,
			GregorianCalendar endDate, int quantity) {
		Map<MaterialType, ArrayList<Material>> availableStock = this
		...
		if (availableStock.get(materialType).size() >= quantity) {
			...
			if (connectedUser instanceof Manager)
				book = new Loan((Manager) connectedUser, materials, startDate,
						endDate);
			if (connectedUser instanceof Borrower)
				book = new Loan((Borrower) connectedUser, materials, startDate,
						endDate);
			loans.add(book);
		}
